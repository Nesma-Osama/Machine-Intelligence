...
class SnakeEnv(Environment[SnakeObservation, Direction]):
    ...
    def actions(self) -> List[Direction]:
        ...
        """
        # if the snake moves in one direction cannot tell him to move in the same and oposite direction
        # if moving horizontally can move only vertically and vice versa
        if self.direction in (Direction.LEFT, Direction.RIGHT):
            orthogonals = [Direction.UP, Direction.DOWN]
        elif self.direction in (Direction.UP, Direction.DOWN):
            orthogonals = [Direction.LEFT, Direction.RIGHT]
        else:
            # if none any direction is possible
            orthogonals = [Direction.UP, Direction.DOWN, Direction.LEFT, Direction.RIGHT]

        # Include NONE to keep moving in current direction
        return [Direction.NONE] + orthogonals
    ...
    def step(self, action: Direction) -> \
        ...
        """
        # if action is none keep moving in the same direction
        if action == Direction.NONE:
            new_direction = self.direction
        else:
            new_direction = action

        # compute new head position with wrapping if it goes out of bounds 
        head = self.snake[0]
        vec = new_direction.to_vector()
        new_x = (head.x + vec.x) % self.width
        new_y = (head.y + vec.y) % self.height
        new_head = Point(new_x, new_y)

        # check if there an apple in the position of the head
        will_eat = (self.apple is not None and new_head == self.apple)

        # check if the snake bites itself
        snake_grid = set(self.snake)
        tail = self.snake[-1]
        if new_head in snake_grid and not (not will_eat and new_head == tail):
            # bite itself -> lose
            self.snake = [new_head] + self.snake
            self.direction = new_direction
            self.apple = None
            observation = SnakeObservation(tuple(self.snake), self.direction, self.apple)
            return observation, -100.0, True, {}

        # if it eat the tail will grow means the head will be in the new position and the tail will be the same
        if will_eat:
            self.snake = [new_head] + self.snake
        else:
            # the head moves to the new position and the tail moves forward
        ...
        if will_eat:
            reward += 1.0
            # check if this ate the last free cell -> win
        ...
        # no win or lose return normal observation
        observation = SnakeObservation(tuple(self.snake), self.direction, self.apple)
        return observation, reward, done, {}
    ...
    def format_state(self, state: SnakeObservation) -> str:
        ...
        apple = None if state.apple is None else tuple(state.apple)
